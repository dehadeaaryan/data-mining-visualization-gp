...
dataset <- read.csv("racing_data.csv")
...
regressor <- randomForest(formula = finishing_position ~ ., 
                          data = selectedData,
                          ntree = 100)
...
new_data <- data.frame(starting_position = 10,
...
predicted_finishing <- sapply(x_grid, function(x) {
  predict(regressor,
          newdata = data.frame(starting_position = x,
...
df_plot <- data.frame(starting_position = x_grid,
